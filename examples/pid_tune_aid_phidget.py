...
class Roaster(object):
    def __init__(self, kp=1, ki=1, kd=2):
...
if __name__ == "__main__":
    ...
    while True:
        time.sleep(1)
        crntTemp = r.roaster.current_temp #self.roaster.current_temp
        crntTempPhidget=r.roaster.current_temp_phidget
        targetTemp = r.roaster.target_temp
        heaterLevel = r.roaster.heater_level

        print(
             " crntTemp: %d, crntTempPhidget: %d,targetTemp: %d, heaterLevel: %d" %
             ( crntTemp,crntTempPhidget, targetTemp, heaterLevel))
    '''
